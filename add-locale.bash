...
echo -n "Enter locale type (block, page, or backdrop): "
read locale_type
...
# Check whether the locale_type is block, backdrop or page
if [ "$locale_type" != "block" ] && [ "$locale_type" != "page" ] && [ "$locale_type" != "backdrop" ]; then
    echo "Locale type must be block, page, or backdrop"
    exit 1
fi
...
# Read the name of the block, backdrop or page from the user input
...
# Confirm such a block, backdrop or page exists
if [ ! -d "src/$locale_type_plural/$locale_name_camel" ]; then
    echo "Block, backdrop or page does not exist. Please choose a different block or page name."
    exit 1
fi
...
# Add the new locale to the locales file in the block, backdrop or page directory
